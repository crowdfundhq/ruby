module PagSeguro
  class Transaction
    ...
    #
    def self.find_by_reference(reference, config)
      SearchByReference.new("transactions", { reference: reference, email: config.email, token: config.token }, nil, config.enviroment)
    end
